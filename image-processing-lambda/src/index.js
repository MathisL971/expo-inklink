...
const lib_1 = __importDefault(require("sharp/lib")); // Ensure Sharp is packaged with the Lambda layer or deployment
const s3 = new aws_sdk_1.S3();
// Environment variables validation at startup for image processing
const PROCESSED_BUCKET_NAME = process.env.PROCESSED_BUCKET_NAME || process.env.BUCKET_NAME;
if (!PROCESSED_BUCKET_NAME) {
    console.error("Target bucket environment variable (PROCESSED_BUCKET_NAME or BUCKET_NAME) is not set for image processor.");
    throw new Error("Target bucket environment variable is not set for image processor.");
}
/**
...
const processImage = async (event) => {
    ...
    for (const record of event.Records) {
        ...
        const originalImageType = imageTypeMatch[1].toLowerCase() === "jpg"
            ? "jpeg"
        ...
        try {
            ...
            if (!originalImage.Body) {
                throw new Error(`Image body is empty for key: ${sourceKey} in bucket: ${sourceBucket}`);
            }
            ...
            const processingPromises = sizes.map(async (size) => {
                let sharpInstance = (0, lib_1.default)(originalImage.Body).resize(size.width, size.height, { fit: "cover", position: lib_1.default.strategy.entropy }); // 'entropy' for better focus
                ...
                if (size.format === "jpeg") {
                    ...
                    console.warn(`Unsupported format ${size.format} for direct conversion, defaulting to JPEG for ${size.name}`);
                    sharpInstance = sharpInstance.jpeg({
                        quality: 80,
                        progressive: true,
                    });
                ...
                console.log(`Uploading processed image: s3://${PROCESSED_BUCKET_NAME}/${newKey}`);
                return s3
                    .putObject({
                    Bucket: PROCESSED_BUCKET_NAME, // Use the PROCESSED_BUCKET_NAME
                    Key: newKey,
            ...
            console.log(`Successfully processed and uploaded derivatives for ${sourceKey}`);
        }
        catch (error) {
            console.error(`Error processing image ${sourceKey} from bucket ${sourceBucket}:`, error);
